  ...
  @else
    <div style="border:1px solid #27272a;border-radius:.75rem;padding:1.25rem;background:rgba(255,255,255,.015);">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1rem;">
        ...
        <div style="grid-column:1/-1;" x-data="imagePicker(@js($this->imageList))">
          <label style="{{ $labelStyle }}">
            Image
            <span style="opacity:.45;text-transform:none;letter-spacing:0;" x-text="'· ' + images.length + ' available'"></span>
          </label>

          @if ($useCustomImage)
            {{-- custom alias mode --}}
            <input type="text" wire:model="imageCustom" placeholder="e.g. ubuntu/25.04 or a local alias" style="{{ $inputStyle }}">
        @else
            {{-- combobox mode --}}
            <div style="position:relative;" @click.outside="openList = false">
              <input type="text" x-model="query" @focus="openList = true"
                @keydown.escape="openList = false" @keydown.arrow-down.prevent="move(1)" @keydown.arrow-up.prevent="move(-1)"
              ...
              <div x-show="openList" x-cloak x-transition.opacity.duration.100ms
                ...
                <div x-show="filtered.length === 0" style="padding:.7rem .8rem;opacity:.5;font-size:.85rem;">No images match.</div>
              </div>
            </div>
          @endif
      ...
      <div style="margin-top:1rem;">
        ...
        <div style="font-size:.72rem;opacity:.5;margin-top:.4rem;">The "power" profile supplies root disk + eth0 on br0.</div>
      </div>
      ...
      @if ($showAdvanced)
        <div style="margin-top:.75rem;padding-top:1rem;border-top:1px solid #27272a;display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1rem;">
          <div>
            ...
            <div style="font-size:.7rem;opacity:.45;margin-top:.25rem;">Count (2) or pin (0-3). Blank = unlimited.</div>
          </div>
          <div>
            <label style="{{ $labelStyle }}">Memory limit</label>
            <input type="text" wire:model="limitsMemory" placeholder="4GiB" style="{{ $inputStyle }}">
            <div style="font-size:.7rem;opacity:.45;margin-top:.25rem;">e.g. 2GiB, 4096MiB, 25%. Blank = unlimited.</div>
          </div>
          <div style="grid-column:1/-1;">
            <label style="{{ $labelStyle }}">Description</label>
            <input type="text" wire:model="description" placeholder="Optional note" style="{{ $inputStyle }}">
          </div>
          <div style="grid-column:1/-1;">
            <label style="{{ $labelStyle }}">Raw Incus config (one key=value per line)</label>
            <textarea wire:model="rawConfig" rows="3" placeholder="limits.cpu.priority=5" style="{{ $inputStyle }}resize:vertical;font-family:monospace;"></textarea>
          </div>
