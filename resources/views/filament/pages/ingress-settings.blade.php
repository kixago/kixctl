<x-filament-panels::page>
    ...
    <div
        style="display:flex; align-items:center; gap:.4rem; margin-bottom:1.25rem; border-bottom:1px solid rgba(255,255,255,.08); padding-bottom:.75rem;">
        @foreach (['network', 'ingress'] as $chip)
            <button type="button" wire:click="$set('tab', '{{ $chip }}')" @class(['fi-btn'])
                style="padding:.4rem .9rem; border-radius:.5rem; font-size:.85rem; font-weight:600;
                    {{ $tab === $chip ? 'background:rgba(59,130,246,.18); color:#93c5fd;' : 'background:transparent; opacity:.7;' }}">{{ __('settings.tab.' . $chip) }}</button>
        @endforeach
    ...
    @if ($tab === 'network')
        <x-filament::section>
            ...
            @if (($resolver['state'] ?? 'absent') === 'ready' && !$provisioning)
                {{-- READY: resolver up. Show status + Rebuild. (Networks CRUD + config
                     surface land here next.) --}}
                <div style="display:flex; align-items:center; gap:.5rem; margin-bottom:.75rem;">
                    <span
                        style="width:.6rem;height:.6rem;border-radius:9999px;display:inline-block;background:#22c55e;"></span>
            ...
            @elseif (($resolver['state'] ?? 'absent') === 'provisioning' || $provisioning)
                ...
                <div style="display:flex; align-items:center; gap:.5rem;">
                    <span
                        style="width:.7rem;height:.7rem;border-radius:9999px;border:2px solid #6b7280;border-top-color:#e5e7eb;display:inline-block;animation:npspin .8s linear infinite;"></span>
        ...
        @if ($provisionToken)
            <div wire:key="console-{{ $provisionToken }}" x-data="provisionConsole(@js($provisionToken), @js($consoleI18n))" x-init="init()"
                ...
                <button type="button" @click="open = !open"
                    ...
                    <span
                        style="font-family:ui-monospace,monospace; font-size:.8rem; opacity:.85; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;"
                        x-text="open ? i18n.heading : last"></span>
                    <span style="font-size:.75rem; opacity:.6; flex-shrink:0;"
                        x-text="open ? i18n.hide : i18n.show"></span>
                </button>
                <div x-show="open" x-cloak x-ref="tail"
                    ...
                    <template x-for="l in lines" :key="l.seq">
                        <div :style="l.stream === 'err' ? 'color:#9ca3af;' : 'color:#93c5fd;'" x-text="l.line">
                        </div>
                    </template>
    ...
    @if ($tab === 'ingress')
        ...
        <x-filament::section>
            ...
            <div style="display:flex; align-items:center; gap:.5rem; margin-bottom:.5rem;">
                <span
                    style="width:.6rem;height:.6rem;border-radius:9999px;display:inline-block;background:{{ $status['ready'] ?? false ? '#22c55e' : '#f59e0b' }};"></span>
    ...
    @if ($provisioning && $provisionToken)
        <div wire:key="netprov-{{ $provisionToken }}" x-data="networkProvision(@js($provisionToken), @js($npI18n))" x-init="init()"
            ...
            <div style="display:flex; align-items:center; gap:.55rem; margin-bottom:.35rem;">
                <span x-show="!terminal"
                    style="width:.7rem;height:.7rem;border-radius:9999px;border:2px solid #6b7280;border-top-color:#e5e7eb;display:inline-block;animation:npspin .8s linear infinite;"></span>
                <span x-show="terminal" x-cloak :style="ok ? 'color:#22c55e' : 'color:#ef4444'"
                    x-text="ok ? '●' : '●'"></span>
                <strong x-text="i18n.title"></strong>
            </div>
            <div x-text="message" style="opacity:.9;"></div>
            <template x-if="ip">
                <div style="margin-top:.3rem; font-family:ui-monospace,monospace; color:#93c5fd;"
                    x-text="(network ? network + ' · ' : '') + ip"></div>
            </template>
            <div x-show="terminal" x-cloak style="margin-top:.55rem; text-align:right;">
                <button type="button" @click="$wire.set('provisioning', false)"
                    style="opacity:.7; text-decoration:underline;">dismiss</button>
            </div>
        </div>
    @endif
